...
namespace Hakoniwa.Assets.EV3
{
  public class RobotController : MonoBehaviour, IAssetController
  {
    ...
    //cicd
    private CICD.StateManager stateManager;
    private GameObject simStart;
    private CICD.Reset reset;
    ...
    public void Initialize()
    {
      ...
      //cicd
      this.reset = GameObject.Find("Hakoniwa").GetComponent<CICD.Reset>();
      reset.Initialize();
    }
    ...
    //cicd
    private void InitCICD()
    {
      GameObject obj;
      obj = GameObject.Find("StateManager");
      this.stateManager = obj.GetComponent<CICD.StateManager>();
    ...
    //cicd
    private void UpdateCICD()
    {
      if (this.stateManager != null)
      {
        //Debug.Log((int)stateManager.GetState());
        this.writer.Set("event_state", (int)stateManager.GetState());
        this.writer.SetRapTime((ulong)stateManager.GetRapTime());
      }
    }

    public bool CanStart()
    {
      this.reader.RefData("start_flag", out start_flag);
      if (start_flag == 0)
      {
        Reset();
        return false;
    ...
    {
      this.reader.RefData("finish_flag", out finish_flag);
      if (finish_flag == 1)
      {
#if UNITY_EDITOR
    ...
    public void Reset()
    {
      ...
      {
        isReset = true;
        GameObject.Find("Hakoniwa").GetComponent<WorldController>().Reset();
        reset.AppReset();
        isStart = false;
      }
